...
class ReportHelperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("報告整理小幫手 v4.0 (PPT 自動生成)")
        self.root.geometry("1200x700")

        # --- Main Layout: Paned Window for resizable panels ---
        paned_window = PanedWindow(root, orient=tk.HORIZONTAL, sashrelief=tk.RAISED)
        paned_window.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        # --- Left Panel: Input & OCR ---
        left_frame = tk.Frame(paned_window, padx=5)
        self.create_left_panel(left_frame)
        paned_window.add(left_frame, stretch="always")

        # --- Right Panel: GenAI Output & PPT Generation ---
    ...
    def create_left_panel(self, parent):
        ...
        # Bottom button
        copy_button = tk.Button(parent, text="步驟 B: 複製 Prompt 與內容至剪貼簿", command=self.copy_for_genai, bg="#2196F3", fg="white", height=2)
        copy_button.pack(fill=tk.X)

    def create_right_panel(self, parent):
        # Text area for GenAI output
        ...
        # Final PPT generation button
        ppt_button = tk.Button(parent, text="步驟 D: 生成 PowerPoint 檔案", command=self.generate_ppt_files, bg="#4CAF50", fg="white", height=2, font=("Arial", 10, "bold"))
    ...
    def paste_from_clipboard(self):
        ...
        try:
            ...
            if isinstance(image, Image.Image): self.process_image_object(image)
            else:
    ...
    def generate_ppt_files(self):
        """核心函式：解析 GenAI 輸出並生成 PPT 檔案"""
        ...
        # 選擇儲存目錄
        save_dir = filedialog.askdirectory(title="請選擇儲存 PowerPoint 檔案的資料夾")
        if not save_dir:
            return

        # 使用正規表達式來分割每個報告
        # `(?=--- 報告 \d+：)` 是一個正向預查，它會匹配分割點但不會消耗分割符
        ...
        for report_text in reports:
            ...
            try:
                # 建立一個新的簡報
                # python-pptx 會使用一個預設的模板，這符合"固定框架"的需求
                # 若要使用公司模板，可改為 prs = Presentation('公司模板.pptx')
                prs = Presentation()
                
                # 選擇"標題與內容"版面配置 (通常是索引 1 或 5，視模板而定)
                ...
                # 產生安全的檔名並儲存
                safe_filename = self.sanitize_filename(title) + ".pptx"
                save_path = os.path.join(save_dir, safe_filename)
                prs.save(save_path)
                generated_count += 1

            except Exception as e:
                messagebox.showerror("生成失敗", f"生成檔案 '{title}.pptx' 時發生錯誤：\n{e}")
                return # 中斷後續操作

        if generated_count > 0:
            messagebox.showinfo("生成完畢", f"成功在指定資料夾中生成了 {generated_count} 個 PowerPoint 檔案！")
        else:
            messagebox.showwarning("未找到報告", "未能在貼上的內容中找到符合格式的報告，請檢查內容是否包含 `--- 報告 1：... ---` 的格式。")
