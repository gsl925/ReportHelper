...
# FINAL, ULTIMATE CHANGE: 引入智能模式偵測，自動適應淺色/深色模式
def process_image_content(image_input):
    """
    使用 Tesseract 進行 OCR，並在之前進行智能影像前處理。
    (增加了偵錯輸出的版本)
    """
    try:
        ...
        # --- 偵錯點 1: 顯示原始圖片 ---
        st.image(image, caption='偵錯：步驟 1 - 原始輸入圖片', use_column_width=True)
        ...
        # --- 偵錯點 2: 顯示前處理後的圖片 ---
        st.image(thresh, caption='偵錯：步驟 2 - 處理後準備送入 OCR 的圖片', use_column_width=True)
        ...
        print(text)
        print(f"--- End of Result (Length: {len(text)}) ---")

        if not text.strip():
             st.warning("偵錯：OCR 引擎回傳了空字串。請檢查上方『處理後準備送入 OCR 的圖片』是否為全白或全黑。")
...
with col1:
    ...
    # 讓 text_area 直接綁定到 ocr_text 這個 session_state 鍵
