...
with col1:
    ...
    st.file_uploader("選項 1: 拖曳或點擊上傳檔案", type=['png', 'jpg', 'jpeg', 'txt', 'msg'], key="file_uploader_key", on_change=handle_file_upload)
    st.write("選項 2: 從剪貼簿貼上圖片")
    paste_info = paste_image_button(label="📋 貼上截圖 (Ctrl+V)", key="paste", background_color="#FF4B4B", hover_background_color="#FF6B6B")
...
with col2:
    ...
    # --- *** STEP 2: MODIFY FILE UPLOADER *** ---
    st.subheader("選項：使用現有簡報範本")
    # 當檔案上傳時，將其值直接存入 session_state
    ...
    if st.button("產生 PowerPoint 報告", type="primary", use_container_width=True):
        if genai_output:
            with st.spinner("正在生成 PowerPoint 檔案..."):
                # --- *** STEP 3: READ FROM SESSION STATE *** ---
                # 從 session_state 讀取範本檔案，而不是從元件的即時回傳值
